...
- [x] 1.1 Create `server/src/main/kotlin/com/example/proyectofinal/models/AdminDtos.kt` — `PageResponse<T>` (items, page, size, totalElements, totalPages), `AdminUserResponse`, `AdminCourseResponse`, and `RoleUpdateRequest`. Omit `createdAt` from user response per design.
- [x] 1.2 Add `listUsers(query: String?, page: Int, size: Int): PageResponse<AdminUserResponse>` to `UserService.kt` — Exposed query with `LIKE "%query%"` on name/email, `LIMIT`/`OFFSET`, separate `COUNT(*)`.
- [x] 1.3 Add `getAllCoursesAdmin(): List<AdminCourseResponse>` to `CourseService.kt` — return each course with `creatorName` and `enrollmentCount`.

## Phase 2: Admin Routes, Wiring & CORS (PR 1)

- [x] 2.1 Create `server/src/main/kotlin/com/example/proyectofinal/routes/adminRoutes.kt` — `GET /admin/users`, `GET /admin/courses`, `PUT /admin/users/{id}/role`, all under `authenticate("auth-jwt")` + `requireAdmin()`. Use `RoleUpdateRequest` for the role body.
...
- [x] 3.2 Create `AdminServiceTest.kt` — service-layer tests for `listUsers` offset/limit/search and `getAllCoursesAdmin` creator-name + enrollment-count aggregation, following existing `ServiceLayerTest.kt` pattern.
