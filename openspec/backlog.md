...
2. **Onboarding asks school year before education level.** Current step order in `OnboardingViewModel`/`OnboardingScreen` is Province → School year → Category (`StudentTrack`, i.e. primario/secundario/secundario técnico). It should ask level (`StudentTrack`) first, then only show the years valid for that level (1-5/6/7 depending on level), not the other way around. This likely means swapping the `SCHOOL_YEAR` and `CATEGORY` steps and re-deriving `availableSchoolYears` from the selected track instead of `ProvinceSchoolCatalog.schoolYearOptionsFor(province)` alone.
3. **Logout button is visible during onboarding.** `OnboardingContent` in `OnboardingScreen.kt:86-92` renders an explicit Logout button in the header. Onboarding is a required step right after registration; it shouldn't offer an exit. Logout should only be reachable from the Profile screen once onboarding is complete (already present in `ProfileScreen.kt`).
